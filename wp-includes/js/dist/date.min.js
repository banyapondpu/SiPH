...
//! license : MIT
//! github.com/moment/moment-timezone
!function(q,o){"use strict";M.exports?M.exports=o(b(174)):(O=[b(27)],void 0===(A="function"==typeof(p=o)?p.apply(z,O):p)||(M.exports=A))}(0,function(M){"use strict";if(!M.tz)throw new Error("moment-timezone-utils.js must be loaded after moment-timezone.js");var z="0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWX",b=1e-6;function p(M,p){for(var O="",A=Math.abs(M),q=Math.floor(A),o=function(M,p){for(var O,A=".",q="";p>0;)p-=1,M*=60,O=Math.floor(M+b),A+=z[O],M-=O,O&&(q+=A,A="");return q}(A-q,Math.min(~~p,10));q>0;)O=z[q%60]+O,q=Math.floor(q/60);return M<0&&(O="-"+O),O&&o?O+o:(o||"-"!==O)&&(O||o)||"0"}function O(M){return function(M){if(!M.name)throw new Error("Missing name");if(!M.abbrs)throw new Error("Missing abbrs");if(!M.untils)throw new Error("Missing untils");if(!M.offsets)throw new Error("Missing offsets");if(M.offsets.length!==M.untils.length||M.offsets.length!==M.abbrs.length)throw new Error("Mismatched array lengths")}(M),[M.name,function(M){var z,b,O=0,A=[],q=[],o=[],c={};for(z=0;z<M.abbrs.length;z++)void 0===c[b=M.abbrs[z]+"|"+M.offsets[z]]&&(c[b]=O,A[O]=M.abbrs[z],q[O]=p(Math.round(60*M.offsets[z])/60,1),O++),o[z]=p(c[b],0);return A.join(" ")+"|"+q.join(" ")+"|"+o.join("")}(M),function(M){var z,b=[],O=0;for(z=0;z<M.length-1;z++)b[z]=p(Math.round((M[z]-O)/1e3)/60,1),O=M[z];return b.join(" ")}(M.untils)+function(M){if(!M)return"";if(M<1e3)return"|"+M;var z=String(0|M).length-2;return"|"+Math.round(M/Math.pow(10,z))+"e"+z}(M.population)].join("|")}function A(M,z){var b;if(M.length!==z.length)return!1;for(b=0;b<M.length;b++)if(M[b]!==z[b])return!1;return!0}function q(M,z){return A(M.offsets,z.offsets)&&A(M.abbrs,z.abbrs)&&A(M.untils,z.untils)}function o(M,z){var b=[],p=[];return M.links&&(p=M.links.slice()),function(M,z,b,p){var O,A,o,c,W,d,R=[];for(O=0;O<M.length;O++){for(d=!1,o=M[O],A=0;A<R.length;A++)q(o,c=(W=R[A])[0])&&(o.population>c.population?W.unshift(o):o.population===c.population&&p&&p[o.name]?W.unshift(o):W.push(o),d=!0);d||R.push([o])}for(O=0;O<R.length;O++)for(W=R[O],z.push(W[0]),A=1;A<W.length;A++)b.push(W[0].name+"|"+W[A].name)}(M.zones,b,p,z),{version:M.version,zones:b,links:p.sort()}}function c(M,z,b){var p=Array.prototype.slice,O=function(M,z,b){var p,O,A=0,q=M.length+1;for(b||(b=z),z>b&&(O=z,z=b,b=O),O=0;O<M.length;O++)null!=M[O]&&((p=new Date(M[O]).getUTCFullYear())<z&&(A=O+1),p>b&&(q=Math.min(q,O+1)));return[A,q]}(M.untils,z,b),A=p.apply(M.untils,O);return A[A.length-1]=null,{name:M.name,abbrs:p.apply(M.abbrs,O),untils:A,offsets:p.apply(M.offsets,O),population:M.population}}return M.tz.pack=O,M.tz.packBase60=p,M.tz.createLinks=o,M.tz.filterYears=c,M.tz.filterLinkPack=function(M,z,b,p){var A,q,W=M.zones,d=[];for(A=0;A<W.length;A++)d[A]=c(W[A],z,b);for(q=o({zones:d,links:M.links.slice(),version:M.version},p),A=0;A<q.zones.length;A++)q.zones[A]=O(q.zones[A]);return q},M})},27:function(M,z){!function(){M.exports=this.moment}()}});